...
- `pnpm test:e2e` — desktop e2e suite (`apps/desktop/e2e/`): launches the built Electron app with an isolated profile (`CALENDAR_USERDATA`) and drives it over CDP — covers rendering, view switching, editor CRUD, drag move/resize/cancel, recurring editing (override drag, scope selector, series split, occurrence delete), visibility toggles. Requires `pnpm --filter @calendar/desktop build` first
- `pnpm test:e2e:ios` — Maestro flows (`apps/ios/e2e/flows/`) against the dev-client build on a booted simulator. Requires the Maestro CLI (`brew install mobile-dev-inc/tap/maestro`), the app installed (`pnpm --filter @calendar/ios ios`), and Metro running (`pnpm --filter @calendar/ios start`)
