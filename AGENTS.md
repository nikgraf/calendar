...
- `packages/sync` — SyncEngine service: per-account sync fibers, pending-op queue (9 op kinds incl. tasks), typed sync errors; Google Tasks watermark sync.
...
- `pnpm test:e2e:ios` — Maestro flows (`apps/ios/e2e/flows/`, 8 flows: launch, navigation, new-event sheet, settings, day swipe, quick-add, create event, task lane) against the dev-client build on a booted simulator. Requires the Maestro CLI (`brew install mobile-dev-inc/tap/maestro`), the app installed (`pnpm --filter @calendar/ios ios`), and Metro running (`pnpm --filter @calendar/ios start`)
