...
- Birthdays are a third all-day kind, not events: `getBirthdaysInRange`
  ...
  read-only summaries in the lane's order (tasks, birthdays, events):
  chips on desktop under the same three-chip cap, dots on iOS under the
  four-dot cap with the counts in the cell's VoiceOver label; tapping a
  cell opens the day, where the full chips live. Both kinds are bucketed
  with `groupByDate` (core), which the iOS all-day lane uses as well.
